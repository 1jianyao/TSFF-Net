    ...
    def _forward_once(self, x1, x2, profile=False, visualize=False):
        ...
        for m in self.model:
            ...
            if m.i < self.fusion_depth:  # backpone1
                    ...
                    if m.type == "models.common.Add":
                        for j in m.f:
                            if j == -1:
                                ...
                                x1 = y[j]
                    elif m.type in ["models.common.Concat3","models.common.Concat4"]:
                        for j in m.f:
                            if j == -1:
                                x2 = x2
                            else:
                                x1 = y[j]
                    elif m.type in ["models.common.AFF","models.common.SKConv2","models.common.AFF_HW","models.common.AFF_CHW",
                                    "models.common.AFF_CHW2","models.common.AFF_CHW3","models.common.AFF_CHW4"]:
                        for j in m.f:
                            if j == -1:
                                x2 = x2
                            else:
                        ...
                        for j in m.f:
                            if j == -1:
                                ...
                                x1 = y[j]
                                    ...
                                    "models.common.Dual_BA2M_M_cat","models.common.Dual_BA2M_AFF_Spa",
                                    "models.common.Dual_BA2M_Concat3","models.common.Dual_BA2M_Concat4",
                                    "models.common.Dual_BA2M_Concat5","models.common.Dual_BA2M_HW"]:
                        for j in m.f:
                            if j == -1:
                                x2 = x2
                            else:
                ...
                if m.type == "models.common.Add":
                    x2 = m(x1, x2)
                # if m.type == "models.common.C2_f":
                #     feature_visualization2(x2, m.type, m.i, 1, 1, 1, "x3{}".format(m.i))
                    # x2 = m(x1, x2)
                elif m.type in ["models.common.Concat3","models.common.Concat4"]:
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, ch2=3, nc=None,
        ...
        if isinstance(m, Detect) or isinstance(m, Decoupled_Detect):
            s = 256  # 2x min stride
            m.inplace = self.inplace
            m.stride = torch.tensor([s / x.shape[-2] for x in self.forward(torch.zeros(2, ch, s, s),torch.zeros(2, ch2, s, s))])  # forward
...
def parse_model(d, ch, ch2):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone1'] + d['backbone2'] + d['head']):  # from, number, module, args
        ...
        # 再backbone2开始和之后
            ...
            if m in {
                Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, C2f,C2f_cbam,C2f_ema,C3_cbam,C2f_ca,C2f_nam,
                C2f_sa,C2f_eca,C2f_CloFormer,C2f_Triplet,C2f_LSK,C2f_DCN,C2f_Ghost,VoVGSCSP,GSConv,C2f_RepGhost,RepGhostModule,C2f_ema2,C2f_GS,C2f_SimAM2,
                C2f_SimAM,C2f_se,C2f_se2}:
                # 需要从第二模态图片重新开始
                ...
                if m in {BottleneckCSP, C3, C3TR, C3Ghost, C3x, C2f,C2f_cbam,C2f_ema,C3_cbam,C2f_ca,C2f_nam,C2f_sa,C2f_eca,C2f_CloFormer,
                         C2f_Triplet,C2f_LSK,C2f_DCN,C2f_Ghost,VoVGSCSP,C2f_RepGhost,C2f_ema2,C2f_GS,C2f_SimAM2,C2f_SimAM,C2f_se,C2f_se2}:
                    args.insert(2, n)  # number of repeats 在第二位置插入Bottleneck的个数n
                    n = 1  # 恢复默认值
            ...
            # Concat2与Concat层结构相同，但是由于最终输出分别存在ch和ch2中
                ...
                for x in f:
                    ...
                    c22 = int(c12 / 2)
                args = [c22, *args[0:]]
            elif m in [AFF,AFF_HW,AFF_CHW,AFF_CHW2,AFF_CHW3,AFF_CHW4]:
                c12 = 0
                for x in f:
                    if x == -1:
                        c2m = ch2[x]
                ...
                args = [c12, *args[0:]]
                c22 = c12 * 2
            elif m in [Dual_BA2M_c, Dual_BA2M_s,Dual_BA2M_M,Dual_BA2M_SE,Dual_BA2M_AFF,Dual_BA2M_AFF_Spa,Dual_BA2M_Concat3,Dual_BA2M_Concat4,Dual_BA2M_Concat5,Dual_BA2M_HW]:
                c12 = ch2[f[0]]
                args = [c12, *args[0:]]
                c22 = c12
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='yolov5s_mix_fusion176.yaml', help='model.yaml')
